...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
        ...
        } catch (CustomJwtException | UserException ex) {
            // 토큰 만료, 서명 오류 등 커스텀 JWT 예외에 대한 직접 처리
            log.warn("[JwtAuthenticationFilter] JWT 예외 발생: {}",
                    ex instanceof CustomJwtException
                    ? ((CustomJwtException) ex).getStatus().getResponseMessage()
                    : ((UserException) ex).getStatus().getResponseMessage());
            ...
            BaseResponse<Object> errorResponse = new BaseResponse<>(
                    ex instanceof CustomJwtException
                    ? ((CustomJwtException) ex).getStatus()
                    : ((UserException) ex).getStatus()
            );
